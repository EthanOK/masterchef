...
contract MasterChefV2 is Ownable {
    ...
    function pendingSushi(uint256 _pid, address _user) external view returns (uint256 pending) {
        ...
        pending = (int256(user.amount * (accSushiPerShare) / ACC_SUSHI_PRECISION) - user.rewardDebt).toUint256();
    }
    ...
    function sushiPerBlock() public view returns (uint256 amount) {
        amount =
            (MASTERCHEF_SUSHI_PER_BLOCK) * (MASTER_CHEF.poolInfo(MASTER_PID).allocPoint) / MASTER_CHEF.totalAllocPoint();
    }
        ...
        if (block.number > pool.lastRewardBlock) {
            ...
            if (lpSupply > 0) {
                ...
                pool.accSushiPerShare =
                    pool.accSushiPerShare + ((sushiReward * (ACC_SUSHI_PRECISION) / lpSupply).toUint128());
            }
